...
| HIGH | 9 | 8 (F-01, F-02, H-4, iOS-F6, H-1 PR #527, iOS-F5 device-verified 2026-07-07, iOS-F9 CLOSED 2026-07-07, H-2/iOS-F11 BOTH halves RESOLVED — Android PR #516/#518, iOS 2026-07-08 iPhone 8 Plus) | 1 (H-3 accepted deviation) |
...
>   Sepolia txid (block 11187337) proved the KEK-gated unlock **FLOW** end-to-end — it did
...
| H-2 / iOS-F11 | HIGH | Android + iOS | **Biometric factor not bound to enrollment set.** An attacker who can add a new biometric (e.g., a coercer who forces a new fingerprint enroll) would otherwise retain access to the KEK-gated vault. **Android — RESOLVED / device-verified (PR #516/#518, 2026-07-01, Pixel 10 Pro XL):** `setInvalidatedByBiometricEnrollment(true)` (`HardwareKekPlugin.kt:199`) confirmed working — delete + re-enroll fingerprint → `KeyPermanentlyInvalidatedException` → app detects → fail-closed unlock refusal → PIN fallback recovered the vault. Recorded as the `_hardware_kek_biometric_reenroll_invalidation` META key in `docs/verified-evidence.json`. **iOS — ✅ RESOLVED / device-verified (2026-07-08, INTERNAL):** `kSecAccessControlBiometryCurrentSet` is correctly set on the SE key ACL (`HardwareKekPlugin.m:96` — see positive confirmations below). Tested on iPhone 8 Plus (iOS 16.7.16, Touch ID, unrestricted — no MDM): enrolled KEK vault + added new fingerprint → SE key invalidated → "Incorrect PIN" (fail-closed, I4) → no unlock, no silent bare fallback. The iPhone 17 Pro Max (MDM-registered) could not run this test (confirmed 2026-07-07); the iPhone 8 Plus was used instead. Both use the same ACL flag. Consistent with `H-NEW-5` (2026-06-27 honest-disable, separate `biometricUnlock.js` PIN cache). | Android ✅ device-verified (PR #516/#518). iOS ✅ device-verified (2026-07-08, iPhone 8 Plus, Touch ID). Both platforms RESOLVED. |
...
2. **Biometric re-enrollment invalidation test** — Android ✅ DONE (PR #516/#518, Pixel 10 Pro XL, 2026-07-01: adding a new biometric invalidated the KEK-gated vault, fail-closed, PIN recovery intact). iOS ✅ DONE (2026-07-08, iPhone 8 Plus, iOS 16.7.16, Touch ID, unrestricted — no MDM): enrolled KEK vault + added new fingerprint → SE key invalidated → "Incorrect PIN" (fail-closed, I4) → no unlock, no silent bare fallback. **Both platforms RESOLVED.**
