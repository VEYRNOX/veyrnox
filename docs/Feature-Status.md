...
- Action Password 2FA parity in decoy/hidden sessions — 🟠 IMPLEMENTATION READY (2026-06-30) — **Decision: Option A (Decoy WITH second factor).** Storage shape landed; enforcement + UI to be wired. The Action Password second factor (§5) currently enforces on the **primary set only**. **H2 storage groundwork is complete:** decoy (`duress.js`) and hidden (`stealth.js`) slots now wrap the seed in a **FIXED-LENGTH multi-seed container** (`makeContainer`/`serializeContainer`) that *can* carry a per-set Action Password record. `setDuressVault` accepts an `actionPasswordRecord`; `stealth.setHiddenActionPasswordRecord` writes one into a hidden slot. **Chaff-length parity resolved:** `makeChaff` sizes every fake blob to `FIXED_LEN`, so real slots (with or without Action Password) and chaff all share one byte-identical constant length (padded by `serializeContainer`). **Implementation scope (next session, ~2-3 hours):** (1) UI to collect Action Password during decoy/hidden setup (`DuressPin.jsx` + `StealthWallets.jsx`), (2) enforce-at-action wiring in `twoFactorGate.js` (read per-set Action Password from unlocked container), (3) tests covering decoy 2FA flow. **Decision rationale:** defense-in-depth for decoy wallet (PIN + second factor, same as primary); consistent security posture across all wallet types. Trade-off accepted: friction under coercion, but stronger protection if decoy is ever compromised outside a coercion scenario. See `docs/h2-threat-model-decision-required.md` for full reasoning. **Note — Passkey method (§5, Method 2) is device-global:** it *does* prompt in decoy/hidden sessions (different model, not per-set). Full design note in `docs/vault-auth-architecture-brief.md` §6b.
