...
| F-09 | RASP | RASP not adversarially tested on rooted/Frida devices — OS-level probes unverified on live targets. **JS wiring layer BUILT (PR #811, 2026-07-11):** `src/rasp/nativeProbe.js` exports `nativeProbeSource` and the `resolveProbeSource` chooser; `SendCrypto.jsx` samples `nativeProbeSource()` at mount via `useEffect`, passes to `resolveProbeSource` → `detect()`. 7 unit tests. Native plugin (`RaspIntegrityPlugin.kt`/`.m`) BUILT on both platforms. Pre-sign gate fail-closed fix: PR #825 (C-01). CAUTION flow + riskReady gate fixes: PRs #832 + #834 (2026-07-12). | ✅ **DEVICE-VERIFIED (FULL, INTERNAL) 2026-07-12.** Device: Samsung Galaxy Note 20 5G (SM-N981B), Magisk v30.7, Android debug build. `RaspIntegrityPlugin` registered; `checkIntegrity()` called via Capacitor bridge (23:40:01): `{"rooted":false,"hookedProcess":false,"emulator":false,"tampered":false}`. `rooted:false` — Magisk Hide at OS-probe level, expected, not a code flaw. `tampered:false` — achieved via `-PRELEASE_CERT_SHA256` Gradle property (production must set this). Pre-sign gate: `TIER.ALLOW` (all signals false) → CAUTION → acknowledged → send. On-chain: Ethereum mainnet txid `0x4556e2e68087d0b75b35504247ed09f011d42614f11b31c5d1423694799da515`, block 25,511,567 (0x1854a8f), status SUCCESS, 0.001 ETH. **Honest gaps:** Frida-hooked device test NOT performed. `rooted:false` on Magisk is probe-level, not a bypass finding. `tampered` requires `RELEASE_CERT_SHA256` in production Gradle. Not independently audited. **iOS RASP (2026-07-13, SEPARATE GAP):** tested on iPhone 8 Plus (iOS 16.7.16, palera1n rootful jailbreak) — `checkIntegrity()` returned all signals false; palera1n NOT detected (false negative). Root cause: path checks blocked by kernel-enforced app sandbox, no Cydia/Sileo by default, no dyld injection. `RaspIntegrityPlugin.m` updated with palera1n-specific path list + C `stat()` syscall check (`checkJailbreakPathsCstat`) + `fork()` check (`checkFork`); `detectJalkbreak` updated to call all four methods. Status: BUILT-UNVALIDATED — compiled, NOT yet re-tested on palera1n device. Additional side findings from this session: Keychain `whenPasscodeSetThisDeviceOnly` → `errSecNotAvailable` (-25291) on palera1n (securityd patched); test-build workaround `whenUnlockedThisDeviceOnly` only, NOT production-patched. Argon2id 192 MiB OOM on iPhone 8 Plus (A11, 3 GB RAM) — open device compatibility gap, not production-patched. |
