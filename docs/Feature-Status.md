...
> **PROVISIONAL pending that audit** (ECC independent third-party audit ran 2026-06-23;
> all findings resolved in PR #340 — see `docs/audit-triage/ecc-independent-audit-2026-06-23.md`).
> Status last verified: 2026-06-23 (ECC independent audit + PR #340).
...
- Hardware wallet (Ledger / Trezor) — ✅ live (`HardwareWalletPage.jsx` + `evm/hw-send.js` + `btc/hw-send.js` + `sol/hw-send.js`; `@trezor/connect-web` + `@ledgerhq/hw-app-eth/btc/solana`). ETH/BTC/SOL address derivation and EIP-1559/PSBT/SOL signing for Ledger (WebHID, Chrome/Edge desktop) and Trezor (Connect popup). I1 preserved; private key never leaves the hardware device. ERC-20 hardware signing and multi-account paths not yet wired. iOS WKWebView fails soft to "WebHID unavailable" card. (PR #246, merged 2026-06-20)
...
## 7. Security — S4 hardening — 🟡 3 of 5 built (incl. local cloud-backup export/import); rest blocked / native / audit-gated
...
> - Encrypted cloud backup — 🟡 the LOCAL encrypt-then-export/import path is BUILT (`vaultBackup.js`, UNAUDITED-PROVISIONAL): the user downloads a ciphertext-only file and restores it by local decrypt. The BACKEND-ESCROW variant (server-side ciphertext target) stays 📋 backend + audit gated — it needs a cloud target (backend was removed) and is key-handling, the catastrophic surface. Needs a backend decision + audit before any build.
