...
| F-09 | RASP | RASP not adversarially tested on rooted/Frida devices — OS-level probes unverified on live targets | 🟡 **BUILT + device-verified (PARTIAL) 2026-07-11, INTERNAL.** `RaspIntegrityPlugin` (Android Kotlin + iOS ObjC, both BUILT) registered and running in Veyrnox process on a real rooted device: Samsung Galaxy Note 20 5G (SM-N981B), Magisk v30.7. Logcat confirmed `Registering plugin instance: RaspIntegrity`. StrongBox-tier KEK enrollment (tier=STRONGBOX, securityLevel=2) + biometric unlock confirmed end-to-end on the rooted device in the same session. **Honest scope (PARTIAL):** `checkIntegrity()` return value and the RASP WARN signal on the Send screen were NOT captured — user did not navigate to Send during the RASP monitoring window; rooted-signal propagation and WARN UI are unit-tested (BUILT) but not observed on this device. Frida-hooked device test not performed. iOS device test not performed (Mac required). Not independently audited. |
