...
- **Security Advisor + Send Preview — Tornado Cash / Lazarus / Blender.io / Sinbad now BLOCK on every EVM chain (2026-08-09, wallet PR #1646 + tip PRs #37/#39) — ✅ BUILT, INTERNAL, live-verified against prod Worker.** Three ordered defects discovered while regression-testing the router `0x8589427373D6D84E98730D7795D8f6f8731FDA16`, all shipped and re-verified end-to-end via curl against the deployed Supabase edge → prod TIP Worker.
  ...
  - **Honest gaps.** BUILT, INTERNAL, not device-verified in the strict on-chain-txid sense (screening advisories, not signing). Three sanction-grade Worker sources (Chainalysis, OpenSanctions, Alchemy tx-sim) remain unset on BOTH staging and production (verified 2026-08-09: every curl to either Worker returns `status: skipped` with `*_API_KEY not configured` for these three; the keys have never been provisioned on the veyrnox-tip project). Not independently audited. **`Ronin Bridge Exploiter` namespace added to `SANCTIONED_LABEL_NAMESPACES` 2026-08-09 (tip PR #40)** — DPRK/Lazarus attribution for the ~$620M Ronin bridge theft (`0x098B716B…F2f96`, OFAC SDN 14 Apr 2022). Curl-verified across ethereum / polygon / arbitrum / bsc; the `sanctioned-address` lane now hits directly on every EVM chain in addition to the pre-existing OFAC-GitHub + GoPlus catches. One transient Polygon `clean` observed on the first request after deploy was Cloudflare KV eventual-consistency (5/5 hit on re-runs after cache warm), not a code path.
