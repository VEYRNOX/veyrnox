...
| F-09 | RASP | RASP not adversarially tested on rooted/Frida devices — OS-level probes unverified on live targets. **JS wiring layer BUILT (PR #811, 2026-07-11):** `src/rasp/nativeProbe.js` exports `nativeProbeSource` and the `resolveProbeSource` chooser; `SendCrypto.jsx` samples `nativeProbeSource()` at mount via `useEffect`, passes to `resolveProbeSource` → `detect()`. 7 unit tests. Native plugin (`RaspIntegrityPlugin.kt`/`.m`) BUILT on both platforms. Pre-sign gate fail-closed fix: PR #825 (C-01). CAUTION flow + riskReady gate fixes: PRs #832 + #834 (2026-07-12). **Android detection improvements (2026-07-13, BUILT-UNVALIDATED):** three new Magisk-Hide-bypass vectors added to `RaspIntegrityPlugin.kt` — `checkProcNetUnix()` (kernel `/proc/net/unix` socket scan, not masked by mount-namespace Hide), `checkSuFromRuntime()` (`which su` via Runtime.exec), and `checkDangerousProps()` (bootloader `ro.boot.verifiedbootstate`/`ro.boot.flash.locked` via getprop). Extended path lists cover KernelSU, Apatch, LSPosed, and newer Magisk artifacts. NOT yet re-tested on the SM-N981B — rebuild + re-run required. | ✅ **DEVICE-VERIFIED (FULL, INTERNAL) 2026-07-12** — original three checks + pre-sign gate flow. **Android detection improvements 2026-07-13: BUILT-UNVALIDATED** (three new Magisk-Hide-bypass checks added to `RaspIntegrityPlugin.kt`; not yet re-tested on device). Device: Samsung Galaxy Note 20 5G (SM-N981B), Magisk v30.7, Android debug build. `RaspIntegrityPlugin` registered; `checkIntegrity()` called via Capacitor bridge (23:40:01): `{"rooted":false,"hookedProcess":false,"emulator":false,"tampered":false}`. `rooted:false` — Magisk Hide at OS-probe level, expected, not a code flaw (the 2026-07-13 new checks are the direct response to this gap, but are unvalidated). `tampered:false` — achieved via `-PRELEASE_CERT_SHA256` Gradle property (production must set this). Pre-sign gate: `TIER.ALLOW` (all signals false) → CAUTION → acknowledged → send. On-chain: Ethereum mainnet txid `0x4556e2e68087d0b75b35504247ed09f011d42614f11b31c5d1423694799da515`, block 25,511,567 (0x1854a8f), status SUCCESS, 0.001 ETH. **Honest gaps:** Frida-hooked device test NOT performed. iOS device test NOT performed (Mac required — iOS palera1n false negative found 2026-07-13, see §iOS RASP). The three new Android checks are BUILT-UNVALIDATED — original 2026-07-12 device-verified status covers the original three checks and the send flow only. Not independently audited. |
