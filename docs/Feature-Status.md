...
- **KEK auto-enroll — onboarding-friction fix (PRs #1298 + #1301, merged 2026-07-21 / 2026-07-22) — INTERNAL, unit-tested only:** ✅ BUILT. **What changed:** the KEK enrollment gate (`KekEnrollmentGate.jsx`) now auto-enrolls silently using the PIN the user just typed, instead of prompting them to re-type it — a new `autoEnrollPin` prop, wired from `WalletEntry.jsx` via a transient ref (`autoEnrollPinRef`, `useRef`, never React state). PR #1298 introduced auto-enroll for fresh creates only; PR #1301 extended it to all three restore paths: phrase import (PIN stashed during Phase 1 `finishPinSetup`), PIN recovery (`realPin` stashed before `setupPin` consumes it), and file restore (unlock PIN stashed on successful `runPinUnlock` when `kekOrigin === 'restored'`). The ref is zeroized (`autoEnrollPinRef.current = null`) on every exit path — success, error, back-navigation, and gate teardown. **Fallback:** on any auto-enroll failure (hardware error, insecure/non-KEK-capable tier), the gate falls back to the pre-existing manual flow with the PIN pad visible — the manual gate's fail-closed behaviour is unchanged and still reachable. **Tap count improvement:** fresh create 5→4, phrase import 6→5, PIN recovery 6→5, file restore 5→4 (screen-level actions, not keystrokes). **Security review:** PASS, no P1s; all P2 advisory items addressed (PR #1298). **Tests:** 14/14 passing (7 auto-enroll tests + 7 pre-existing gate tests, all green). **Honest scope:** BUILT = code-complete + tests green, INTERNAL review only — this is a UI/UX onboarding-flow change with no new signing or key-derivation logic (the underlying `combineKek`/enroll path is unchanged), so no on-chain verification applies and none is claimed; NOT independently audited.
...
- Native secure storage (M2a done; M2b provisional, app-layer) — 🟡 (OS-enforced ACL / Enclave-StrongBox binding = M2c/M2d — 🟡 **iOS M2c BUILT (behind flag, NOT device-verified, PRs #690/#695, 2026-07-07; hardened further by PR #1098, 2026-07-17 — deleteWrappingKey intent gate, stale-key check, versioned key tag)**; **Android M2d-1a/1b/1c/1d BUILT (behind flag, NOT device-verified, PRs #1116/#1131/#1141 + M2d-1d this branch, 2026-07-17/18)** — Capacitor plugin scaffold, capability probe, real StrongBox/TEE `createWrappingKey`, real `BiometricPrompt`-gated `wrap()` AND `unwrap()` (unwrap adds `M2D_CIPHERTEXT_TAMPERED` distinct from `M2D_UNWRAP_FAILED`, and pre-cipher `M2D_MALFORMED_BUNDLE` for base64/unpack failures; response shape `{ blob: '<base64>' }` matches shared JS wrapper + iOS bridge). M2c adds the Swift Secure Enclave key-wrap plugin + capability-detected JS seam, all gated behind `M2C_HARDWARE_WRAP_ENABLED=false` (native behaviour byte-identical to M2b until a physical-iPhone device test passes); Android is gated behind the parallel `M2D_ENABLED=false`. See `docs/audit-triage/m2c-enclave-device-test.md`, `docs/audit-triage/m2d-strongbox-device-test.md` (both STATUS: NOT RUN), and the M2c/d decision note — full per-PR detail in the F-01/F-02 row below.)
...
- **Security Advisor — multi-source threat aggregator + honest UNKNOWN verdict (2026-08-08, PRs #1614/#1615/#1617/#1621/#1622 + local commit 18f023f9) — ✅ BUILT, INTERNAL, not device-verified, not independently audited, no on-chain txid.** The wallet's Security Advisor previously called TIP's single-source `screen` endpoint, which defaulted absence-of-data to `verdict:"allow"` — an OFAC-sanctioned address surfaced as "CLEAR" because the underlying OFAC ingestion never worked (Treasury.gov's Akamai returns HTTP 525 to Cloudflare Workers). Complete overhaul on the TIP side, wallet side consumes the new shape:
  - **Address extraction covers all 4 chain families.** `src/components/SecurityAdvisor.jsx` and `src/api/tipScreen.js` now share a `CHAIN_ADDRESS_PATTERNS` table detecting EVM (0x…), BTC (`bc1…` + legacy `1/3…`), and SOL (base58). Previously the regex was EVM-only, so a pasted BTC or SOL address surfaced as "not connected" and never reached the screening path (commit `d0e3a076`).
  - **5-category verdict shape.** TIP now runs sanctions + phishing + hack registry + contract risk + tx simulation in parallel and returns `verdict ∈ {allow, block, warn, indeterminate, unknown}` plus a `sources_consulted[]` audit trail. Priority: sanctions > phishing > hack > contract-risk > tx-simulation > policy > threat > sim > allow > unknown. The wallet passes the array through unchanged and `SecurityAdvisor.jsx`'s `ScreeningVerdict` component renders an expandable "Sources consulted" trace beneath each verdict.
  - **Honest UNKNOWN verdict.** When ALL categories return `unavailable` (a source outage, no key, or DB miss on every provider), the wallet renders an amber CAUTION reading "verify independently" instead of green CLEAR. `verdictToRiskLevel` maps `unknown → medium` and `requiresAcknowledgment` includes it — the pre-sign gate now honours it. `tipClient.js` + `tipScreen.js` accept the new shape. I4 codified: absence of evidence is no longer treated as evidence of absence.
  - **Local signed IOC push for offline + I3 deniability (commit `18f023f9`, local — pushed on branch merge).** `src/lib/localIocCache.js` verifies an Ed25519-signed manifest from TIP (`/api/v1/manifest`, 1/hr rate limit), stores dedup'd hits in IndexedDB (`veyrnox-ioc-cache`), and `tipScreen.js` calls `tryLocalScreen()` FIRST — a local hit shortcuts the network round trip and works offline. **Deniability path is local-only, no network fallback** — a decoy session never calls TIP. `WalletProvider.jsx` refreshes the manifest on unlock (gated on non-deniability); `panic.js` calls `eraseIocCacheDatabase()` from `panicWipeLocal()`. Public key hardcoded in the wallet: `MCowBQYDK2VwAyEALgWYjSGQY+WULES5PtMn/U04jfqPOCPiELl5er6f8CQ=`; TIP-side signing secret `IOC_MANIFEST_PRIVATE_KEY` set on the `veyrnox-tip-production` Worker (verified: `/api/v1/manifest` returns an 88-char Ed25519 signature).
  - **I3 abort of in-flight Advisor stream (PR #1617).** If the user triggers duress/hidden mid-stream, the SSE reader is aborted before any further tokens land — no partial real-session state reaches a decoy screen.
  - **Honest scope + gaps.** BUILT, INTERNAL. Not device-verified in the strict on-chain-txid sense (screening advisories, not signing). Sanctions triple LIVE: OFAC via `0xB10C/ofac-sanctioned-digital-currency-addresses` GitHub mirror (Treasury.gov Akamai-blocked), OpenSanctions (skipped without key), Chainalysis (skipped without key). Phishing feed: ScamSniffer (2,530 addresses). Hack registry: ETHrotten (1,902 addresses; excludes phishing/OFAC/Tornado/poisoning so it does not double-count). Contract risk: GoPlus `address_security` (no key required, ETH-only). Tx simulation: Alchemy `alchemy_simulateAssetChanges` — code wired, activates when `ALCHEMY_API_KEY` is set on the Worker (currently unset, so this leg reports `unavailable` and contributes to `UNKNOWN` when nothing else fires). Etherscan labels: 29,772 attribution rows (`brianleect/etherscan-labels` mirror) as an info-only source. Not independently audited. Full verification report: `docs/threat-aggregator-e2e-2026-08-08.md`.

- **Security Advisor + Send Preview — Tornado Cash / Lazarus / Blender.io / Sinbad now BLOCK on every EVM chain (2026-08-09, wallet PR #1646 + tip PRs #37/#39) — ✅ BUILT, INTERNAL, live-verified against prod Worker.** Three ordered defects discovered while regression-testing the router `0x8589427373D6D84E98730D7795D8f6f8731FDA16`, all shipped and re-verified end-to-end via curl against the deployed Supabase edge → prod TIP Worker.
  - **D1 (tip #36 → #37): aggregator ignored etherscan-labels attribution.** `etherscan-labels` fetched `Labelled: Tornado.Cash: Donate` for the router and the composite verdict emitted `verdict:"allow"`. OFAC-GitHub's SDN mirror lists the pool contracts but not the router, so no other source hit either. Fix: `SANCTIONED_LABEL_NAMESPACES` (`tornado.cash`, `lazarus`, `blender.io`, `sinbad`) — case-insensitive namespace match on the label; a hit upgrades `sanctionsResult.hit = true`, which `composeVerdict` already treats as an irrevocable BLOCK. Curated list, tight scope — adding an entry requires a citable sanctioning authority + effective date.
  - **D2 (wallet #1645 → #1646): Send Preview sent `chain: 'evm'` where the Worker expects a real slug.** `SendCrypto.jsx` passed the placeholder `'evm'` to `screenTransaction`, and both `etherscan-labels` and `goplus` gate on the chain string — every EVM Send silently `status: skipped` on the two sources most likely to catch address-based sanctions, and the whole Advisor path (which correctly sent `'ethereum'`) rendered BLOCKED for the same address while Send Preview rendered CLEAR. Fix: `src/pages/sendCryptoTipChain.js#resolveTipChain(family, networkKey)` maps every wallet EVM key (mainnet/sepolia/polygon/polygonAmoy/arbitrum/arbitrumSepolia/optimism/optimismSepolia/avalanche/avalancheFuji/bnb/bnbTestnet) to the Worker's slug — critically `bnb → bsc`, and every testnet → its mainnet slug because OFAC/label lookups are address-based and the same seed sends on mainnet. Unknown keys fall back to `'ethereum'`, never the inert `'evm'` (I4).
  - **D3 (tip #38 → #39): D1's sanctioned-namespace check only ran on `chain: ethereum`.** Once D2 sent the right per-chain slug, cross-chain sends to the SAME sanctioned address (Polygon MATIC, BSC BNB, Arbitrum, Optimism, Avalanche, Base) started returning `verdict: allow` — `etherscan-labels` skipped for non-ETH and no other lane caught it. But OFAC sanctions apply to the address, not the chain — same private key, same address, different chain → same sanctioned counterparty. Fix: split the KV lookup into two lanes keyed off the SAME rows — `kvLabelLookup` ('etherscan-labels') stays ETH-only for attribution; new `kvSanctionedAddressLookup` ('sanctioned-address') runs on every EVM chain (`ethereum`/`polygon`/`arbitrum`/`optimism`/`avalanche`/`bsc`/`base`) and emits `hit` iff the namespace is on the curated list. Non-EVM (`btc`/`solana`) skipped. Two separate source rows in the UI trace so a Polygon-send trace reads `etherscan-labels: skipped [...] · sanctioned-address: hit [Tornado.Cash: Donate]` — honest about which lane matched.
  - **Live end-to-end verification (deployed prod Worker, 2026-08-09).** curl matrix — Ethereum/Polygon/Arbitrum/BSC → Tornado router: all BLOCK, `sanctioned-address hit`. Polygon → Vitalik: ALLOW, no false-positive. Advisor sim UI: `0x8589…FDA16` → red **BLOCKED** banner with 11 sources consulted (screen-captured). Vitalik in same session: green CLEAR. Send Preview sim retest deferred (wallet-restore state on the sim between sessions).
  - **Honest gaps.** BUILT, INTERNAL, not device-verified in the strict on-chain-txid sense (screening advisories, not signing). Source-key decisions locked 2026-08-09 after evaluating the three previously-`skipped` sources: **Alchemy tx-sim LIVE** on both staging and production (`alchemy_simulateAssetChanges` returns real answers — curl-verified 2026-08-09 with `Simulation succeeded, N asset change(s), no drain detected` on a plain transfer probe). **Chainalysis PERMANENT SKIP** — free Sanctions Screening API tier discontinued; only paid enterprise contract (5-figure annual minimum, quote-only) remains, and OFAC-GitHub + our `sanctioned-address` lane cover the same address space for the entities the wallet cares about (Tornado / Lazarus / Blender.io / Sinbad / Ronin). **OpenSanctions PERMANENT SKIP for the API** — 30-day trial only, then paid; bulk dataset is CC-BY 4.0 downloadable and could be self-hosted on the Worker as a future improvement if PEP screening becomes in-scope. Not independently audited. **`Ronin Bridge Exploiter` namespace added to `SANCTIONED_LABEL_NAMESPACES` 2026-08-09 (tip PR #40)** — DPRK/Lazarus attribution for the ~$620M Ronin bridge theft (`0x098B716B…F2f96`, OFAC SDN 14 Apr 2022). Curl-verified across ethereum / polygon / arbitrum / bsc; the `sanctioned-address` lane now hits directly on every EVM chain in addition to the pre-existing OFAC-GitHub + GoPlus catches. One transient Polygon `clean` observed on the first request after deploy was Cloudflare KV eventual-consistency (5/5 hit on re-runs after cache warm), not a code path.

- **`rpc-sim` public-RPC simulate lane for AVAX + BSC (2026-08-09, tip PRs #45 + #46 hotfix) — ✅ LIVE.** Alchemy's `alchemy_simulateAssetChanges` doesn't cover AVAX or BSC at any tier, and Alchemy's entire Transaction Simulation API family is being **deprecated 30 September 2026** (verified in Alchemy docs 2026-08-09 — 52 days out at time of writing). Triaged every affordable alternative: **GoPlus** — free-tier `is_simulated: false` on ETH/BSC even for well-formed ERC-20 calldata, AVAX chain_id rejected outright, paid tiers start $199/mo. **Tenderly** — Free tier has no API access at all (dashboard exposes no Access Keys UI); paid Console is quote-only, industry norm $200-500+/mo. **Chainalysis** — sales-led enterprise, quote-only, $8-15k/yr minimum. Landed on `eth_estimateGas` via public JSON-RPC (`bsc-rpc.publicnode.com` + `avalanche-c-chain-rpc.publicnode.com`, both already in the wallet's CSP allowlist). Source `rpc-sim` deliberately scoped to alchemy-uncovered chains only — skips ETH/POLY/ARB/OP/BASE to avoid duplicate load. Signal: `eth_estimateGas` succeeds → `clean` with decoded gas; RPC revert error → `clean` with extracted revert reason (same policy as `alchemy-sim` — would-revert is the SEND's problem, not a screening BLOCK); HTTP/timeout → `error` (I4 CAUTION, never fake-clean). Live-verified against prod Worker: AVAX + BSC → `rpc-sim clean — Estimated gas 21000, no revert`; Arbitrum → `rpc-sim skipped, alchemy-sim clean`. **Honest gap:** NOT decoded-asset-change simulation — users on AVAX/BSC see "estimated gas + no revert" rather than the itemised asset changes Alchemy returns on ETH/POLY/ARB/OP/BASE. Trace-based decoding is a follow-up. **Tx-sim coverage: 8/10 assets** (was 6/10; remaining 2 = SOL, needs different simulate primitive; BTC, no equivalent primitive, uses local `btcSim`). Also self-heals when Alchemy's method dies Sept 30 — the `rpc-sim` scope guard is a trivial one-line lift away from covering ETH/POLY/ARB/OP/BASE as a hard fallback.
...
- **H-3 (2026-07-20) — duress setup didn't clear a pre-existing real-PIN biometric cache — ✅ FIXED (PR #1261, `f3358c2c`).** Internal audit (`docs/audit-2026-07-20-weekly.md`) found the PR #613 guard above (`shouldAutoCacheTypedPin`) only protects the *decoy-first-then-unlock* order; if a user enabled real-PIN biometric unlock BEFORE ever configuring a Duress PIN, `setDuressPin()` provisioned the decoy vault but never called `clearUnlockSecret()` — the cache still held the REAL PIN, so Face ID could still open the REAL wallet on the exact coercion path the feature exists for (I3 + I4). **Fix:** `setDuressPin()` now force-clears the biometric cache and drops the cache preference BEFORE provisioning the decoy (fails closed — if the secure store refuses to clear, no decoy is written); if the user opts into "Face ID opens the Emergency wallet" the caller immediately re-provisions the cache to the decoy secret. A new installed-base guard, `src/lib/duressBiometricGuard.js` (`shouldDisarmBiometricUnlock` / `enforceDuressBiometricInvariant`), is wired at lock-screen mount to catch devices that configured a duress PIN before this fix shipped. `shouldAutoCacheTypedPin` restored to key on the `veyrnox-duress-configured` marker (the stale comments this regression left at `WalletEntry.jsx:761-763` claiming the old, removed guard are corrected). BUILT / unit-tested, INTERNAL — NOT device-verified, NOT independently audited, no on-chain txid.
...
- Encrypted personal backup (ciphertext only) — ✅ LOCAL encrypt-then-export/import **BUILT · shipping-approved (internal audit 2026-07-05, sufficient for mainnet)**; `PersonalBackup.jsx` + `src/wallet-core/vaultBackup.js`: the vault is sealed under a SINGLE combined credential (backup password `‖ 0x1F ‖` backup PIN, both required) via the live Argon2id+AES-GCM vault primitive, round-trip-verified before download, and restored by local decrypt. No cloud transport — the user stores the ciphertext file in their own cloud. The BACKEND-ESCROW variant remains 📋 **backend + audit gated and not built** (no cloud target — backend was removed — and key-handling is the catastrophic surface; the audits did not green-light an unbuilt escrow design).
  - **2026-09-01: replaced the two-parallel-seals model with a single combined seal.** Before today the envelope carried BOTH a `seals.password` and a `seals.pin`, each independently decryptable — either alone unlocked the vault. The 2026-08-16 audit (PR #1834) had bumped the PIN branch from 8→12 digits to widen its offline-crack surface (~10^8→~10^12), because an attacker with the file attacks the WEAKER seal and password strength was irrelevant to that door. The 12-digit floor was a UX stopgap: it broke parity with the rest of the app (8-digit PinPad everywhere) and never touched the underlying two-doors weakness. The rewrite (`BACKUP_VERSION` 1→2, `BIN_VERSION` 2→3) collapses to ONE seal keyed off `Argon2id(password ‖ 0x1F ‖ pin)` — password floor raised to 16 chars (matches `RestoreFromShares.jsx` passphrase floor from PR #1834), PIN back to exactly 8 digits, BOTH required to restore. Combined entropy ~121 bits, Argon2id-slowed. The "PIN in a pinch" recovery affordance is intentionally gone: forgetting either kills the backup. No legacy read path — pre-launch, no production users, verified by owner; a stale legacy `.enc` will fail `parseBackupFile`. `decryptPasswordSeal` + `decryptPinSeal` REMOVED, replaced by `decryptBackupSeal(env, password, pin)`. Restore UI (`RestoreFromFile.jsx`) drops the "password OR PIN" toggle for a single "both required" prompt. Tests updated (`vaultBackup-opacity`, `vaultBackup-ios`, `vaultBackup-return-shape`, `vaultBackup-dead-export`, `vaultBackup-finalisePinRestore-validation`, `RestoreFromFile.test`, `RestoreFromFile.funky.test`, `PersonalBackup.freshRasp.test`, `g4-callsite-pins`). BUILT, INTERNAL — no independent audit, no real-device restore round-trip.
...
- Android native (Capacitor) — ✅ runs on real devices (StrongBox KEK unlock and in-app
  ...
  internal testing LIVE (2026-07-22):** upload-key reset approved, release 5 (1.0) published
  to internal testing track, RASP `detectTamper` verified clean on stock Pixel 10 (PR #1313
  fixed silent `RELEASE_CERT_SHA256` misconfiguration — `gradle.properties` debug fingerprint
  default + wrong SHA-256 in runbook), Play Billing (IAP) device-verified on internal track.
  CI secrets (`KEYSTORE_BASE64`, `KEYSTORE_PASSWORD`, `KEY_ALIAS`, `KEY_PASSWORD`,
  `RELEASE_CERT_SHA256`) updated for the new upload key. versionCodes 1–5 consumed; next
  upload must use 6+
...
- **In-app subscription billing (Safety Plus tier, $5.99/mo) — ✅ BUILT, unit-tested
  ...
  **2026-07-17 — Play launch prep landed (PRs #1187–#1192).** Five PRs closing the Windows-side blockers on the Google Play submission path. PR #1187 (`12c33efd`) wires the privacy policy URL into the app — `LandingPage.jsx:319` footer link and a new `PRIVACY_POLICY_URL = "https://veyrnox.com/privacy"` section in `TermsLegal.jsx` (in-app, reachable from Settings, seen by every Play install). Play's crypto-app policy reviewers actively check whether the app itself surfaces a privacy policy; previously two `href="#"` placeholders and zero in-app privacy content. Honest gap: `https://veyrnox.com/privacy` must be publicly resolvable before Play submission (owner-hosted, outside this repo). PR #1188 (`8a00a7da`) — Android manifest: added `<uses-permission android:name="android.permission.CAMERA" />` + `<uses-feature android:name="android.hardware.camera" android:required="false" />` (fixes a real bug — `QRScanner.jsx` calls `getUserMedia({video})` through Capacitor's WebView; without the manifest permission, `BridgeWebChromeClient.onPermissionRequest`'s `permissionLauncher.launch(CAMERA)` silently fails and QR scanning breaks — users tapping "Scan QR" hit the catch block "Camera access denied") plus a justifying comment above `RECORD_AUDIO` mirroring the biometric one (Voice Commands feature; off-device speech recognizer; I3 suppression in decoy/hidden sessions). Regression test `src/__tests__/android-manifest.test.js` pins 6 static manifest invariants (CAMERA declared, `<uses-feature>` declared, RECORD_AUDIO declared, both permissions have justifying comments naming their features). 6/6 green. Honest gap: the CAMERA-fix "provably broken today" claim is code-and-static-analysis correct but not yet device-verified — needs a real Android device retest after the AAB rebuild. PR #1189 (`87349241`) — `docs/play-launch/data-safety-form.md` — copy-paste-ready DRAFT of the Play Console Data Safety form. Every answer cross-referenced to a file/line. Two owner-decisions resolved from code (Voice Commands defaults to OFF via `VoiceContext.jsx:40 useState(false)`; no third-party crash-log SDK present per grep of `package.json`). Seven ⚠ OWNER-DECISION items remain (voice audio classification, encrypted-backup export declaration, RC App User ID as "Device or other IDs" — recommended yes, Families Policy — recommended not-for-children, Independent security review attestation — recommended NO per CLAUDE.md I4 hard rule, data-deletion URL, OpenRouter LLM call in release build). PR #1191 (`46721d10`) — `docs/play-launch/store-listing.md` — copy-paste-ready DRAFT of every Play store listing text field. App name `Veyrnox` (7/30), short description `Self-custody crypto wallet. Your keys stay on your device. Coercion-resistant.` (77/80), full description 6 sections (2,608/4,000) — positioning, 10 mainnet-live assets by name, security stack in Play-reviewer-friendly terms, coercion resistance with honest limits, what Veyrnox is NOT, Safety Plus + seed responsibility disclosure. Financial products declaration: non-custodial wallet=Yes; exchange/mining/ICO/custodial=No. Assets NOT drafted (need running app — Sunday-on-Mac): icon 512×512, feature graphic 1024×500, 6 recommended phone screenshots. PR #1192 (`65a9d2ec`) — `android/app/build.gradle` versionCode 3→4, versionName "1.0.2"→"1.0.3". Two-line bump; Play Console already has v3 uploaded (2026-07-13, Internal testing = Draft, not rolled out), the next AAB build has to be a higher versionCode or Play rejects the upload. Also corrects an earlier readiness-note error that claimed "no AAB uploaded" — AABs v1/v2/v3 exist; v3 is Active in Play Console's bundle storage but not rolled to any track and is stale (predates every meaningful PR from today). **All 5 PRs BUILT / unit-tested only, INTERNAL — NOT device-verified, NOT independently audited, no on-chain txid.** Play launch remaining gates: (1) Mac-Sunday `mobile:build:release` + `./gradlew bundleRelease` → v4 AAB from current `main` → upload to Play Console; (2) complete App content declarations in Play Console (Data Safety per #1189 draft, Content Rating IARC, Target Audience, Financial Features per #1191 draft §"Field 8"); (3) 6 screenshots + icon 512² + feature graphic 1024×500 (needs running app); (4) tester list added in Play Console → Internal testing → Testers; (5) rollout. Independent security audit remains outstanding per CLAUDE.md hard rules — owner-decision on whether to Internal-Test / Closed-Test / Production-launch before it lands.
...
- **⚠ DISCOVERABILITY GAP — Veyrnox is paste-only, NOT a listed/launchable wallet
  ...
  **Veyrnox does NOT appear in dApps' "select your wallet" modals**, because two things are
  ...
  downstream of the Apple org conversion (Play upload-key reset resolved 2026-07-22;
  internal testing track is live). Coinbase Wallet
...
- **⚠️ OPEN finding (2026-07-20 weekly H-1, `docs/audit-2026-07-20-weekly.md`) — session-approval RASP gate is dead code.** `WalletConnectProvider.jsx` `handleApproveSession` (`:771-772`) checks `gate.blocked`/`gate.sentence`, but `presignGateOrReject()` only ever returns `{proceedAllowed, rejectCode}` — those two properties are permanently `undefined`, so every WC session **approval** proceeds regardless of RASP tier, including a hard `TIER.BLOCK` from a rooted/hooked/emulated device. Scoped to session approval only — the three signing chokepoints listed above all correctly read `proceedAllowed` and remain fail-closed, so this bug alone does not yield a signature or broadcast, only a live connection + address disclosure to a hostile dApp on a compromised device. Shipped broken in the commit that introduced it; no test exercises the branch. **Fix (`!gate.proceedAllowed`) is open in PR #1276, code-complete, CI running, NOT yet merged to main** — do not treat this row as closed until it lands.
...
## 2026-07-20 Deniability hardening + duress biometric fix + accessibility pass — PRs #1261, #1262, #1268, #1274 (#1276 open)

> ⚠️ BUILT · unit-tested · INTERNAL — NOT device-verified, NOT independently audited. No
> on-chain txid. Findings sourced from a same-day branch review + internal weekly audit
> (`docs/audit-2026-07-20-weekly.md`) — INTERNAL AI-driven static analysis, not the
> outstanding independent third-party audit.

| PR | Merge commit | Finding / fix | Section |
|---|---|---|---|
| #1261 | `f3358c2c` | **H-3 (HIGH, I3+I4):** duress setup didn't clear a pre-existing real-PIN biometric cache — the vulnerable state was the DEFAULT on native (onboarding offers biometric ON by default). Fixed in `setDuressPin()` (clears cache + drops preference before provisioning the decoy) + new `src/lib/duressBiometricGuard.js` installed-base guard + `shouldAutoCacheTypedPin` restored to key on `veyrnox-duress-configured`. | §6 (duress bullet) |
| #1262 | `d7f00751` | **C-1 (CRITICAL, I3):** More-drawer "Recent" tiles named `/duress-pin`, `/stealth-wallets`, `/panic-wipe`, rendered in decoy sessions, survived lock, survived panic wipe. Fixed: `useRecentPages.js` write+read gated on `isDeniabilityOrDemoActive()` (fail-closed); `clear()` on `APP_LOCK_EVENT`; `panic.js` gains a `sessionStorage` sweep (`SESSION_RESIDUE_KEYS`/`clearSessionResidue()`). **K-2 (I4+I3):** `ReferralTracker.syncCount` coerced a failed/null API read to `0` and wrote a fake `{tier:'none',paidCount:0}` synced state to shared localStorage — a failure rendered as success, and a decoy session mutating real referral state. A Codex second pass on the same PR additionally found the tracker page read/wrote real referral state before any deniability gate; fixed to render a neutral empty state indistinguishable from a new user in decoy/demo sessions. | "Referral deniability status" subsection below |
| #1268 | `e8cf2775` | **S-1 (I4):** PR #1243 (2026-07-19), which scrubbed internal jargon from `Documentation.jsx`, also deleted user-facing security caveats (PIN offline-exhaustion, Hardware Key Protection being opt-in, Hardware Wallet not device-tested, referral egress disclosure) while the affected items stayed marked "Available". Restored the plain-language caveats + a status legend; added a regression test that pins them in place. | — (Documentation page, no dedicated section above) |
| #1274 | `4c6f6f44` | Accessibility: mobile bottom-nav was keyboard-unreachable (roving `tabindex` with no arrow-key handler — dropped tab semantics for plain nav buttons with `aria-current` instead); More-drawer close button was unlabelled + drawer now has `role="dialog"` (deliberately no `aria-modal`, since it does not trap focus) + Escape-to-close; `HDWalletManager` balance live regions made persistent; dangling `aria-controls` fixed on `HDWalletManager` + `TermsLegal`; `ReferralTracker` progressbar semantics + labelled redeem input; Subscription billing radiogroup gains arrow-key nav. | §11 (Platform / app shell) |

**Open, not yet merged — PR #1276** (branch `claude/fix-h1-wc-session-gate`): H-1 fix for
the WalletConnect session-approval RASP gate (see §12 above for the finding detail and the
"do not treat as closed" caveat). Code-complete, tested, CI running.

**No action taken, correctly — H-2:** the same weekly audit's ColdSign WARN-tier
biometric-step-up finding was not fixed and no new tracker row was opened for it, because
`src/pages/ColdSign.jsx` is unreachable dead code (no route, no import, nothing sets
`location.state.coldSend`) and the underlying gap is already tracked as weekly M-5
(2026-07-14, §7 above).

**Referral deniability status (new subsection, closes a documentation gap):** the referral
system (built PRs #1194, #1195, #1235 — see `docs/CLAUDE-audit-archive.md` for the
original build history) had no entry in this file. As of PR #1262 it is: BUILT, deniability-
hardened — `ReferralTracker.jsx` gates real-state reads/writes on `isDeniabilityOrDemoActive()`
and `syncCount()` no longer converts a failed sync into a fake success state. INTERNAL,
