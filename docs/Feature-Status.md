...
| iOS-F5 | HIGH | iOS | H factor in `NSData` not zeroed post-decryption in `HardwareKekPlugin.m`; requires `NSMutableData` patch. | 🟡 PARTIAL mitigation — MERGED (PR #526, commit `34289591f`; follow-up #531 removed a UB memset). The native `NSMutableData` H buffer IS zeroed (`resetBytesInRange`, `HardwareKekPlugin.m:294`). **COMPILE-VERIFIED 2026-07-07 (PR #705, `.github/workflows/ios-compile-check.yml`, macos-latest/Xcode 26.5):** `HardwareKekPlugin.o` compiled and found in DerivedData — `NSMutableData` + `resetBytesInRange` edit is syntactically and semantically valid. `BUILD SUCCEEDED`. **HONEST SCOPE (source review 2026-07-02, PR #652):** H is first base64-encoded into `hB64`, an immutable `NSString` handed to the Capacitor bridge and copied into the JS-side string — both architecturally unzeroable, so a base64 form of H persists until GC/overwrite. This is inherent to the bridge (best-effort native wipe only), NOT a code defect → residual severity LOW–MEDIUM (scope-honesty note, not an open code defect). Do NOT document as "H fully zeroed from the heap". Still NOT device-verified on a Mac: the heap-dump `leaks`/Instruments check is outstanding and should expect the base64 residue (verify only that the raw `NSMutableData` plaintext buffer is gone). Acceptance criteria: Section A of `docs/hardware-audit-handoff.md`. |
| iOS-F9 | HIGH (evidence gap) | iOS | SE-unlock app-trace was previously uncaptured (iOS 26 suppresses third-party NSLog from external tools); proof was architectural + OS-daemon corroboration only. | ✅ CLOSED (2026-07-02) — the literal app trace was captured on iPhone 17 Pro Max (iOS 26.5.1) via a `#if DEBUG` `os_log(public)` rebuild + `pymobiledevice3 syslog live`: `[VEYRNOX-KEK] getHardwareFactor: SUCCESS — Face ID passed, H recovered (32 bytes)` (~2.8 s after the Face ID prompt). Evidence key `_ios_f9_se_unlock_trace_captured` in `verified-evidence.json`; DEBUG-only (never logs H, only its 32-byte length). **Honest residual (does NOT reopen F9):** the trace was not additionally bound to a same-session unlock→send txid, and it is UNAUDITED. iOS overall stays device-verified **PARTIAL** because the biometric re-enrollment invalidation test (H-2/iOS-F11, device-blocked) + independent audit remain. |
