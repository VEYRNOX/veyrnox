...
| M-9 | MED | Short-PIN exhaustion time not disclosed to users — an 8-digit numeric PIN has ~100M combinations; offline exhaustion time at real Argon2id cost is not surfaced in UI. Safari users additionally have no hardware factor (PRF not available). | 🟡 BUILT (docs disclosure, #753): `SECURITY.md` "PIN strength and offline attack resistance — honest disclosure" section added, disclosing the ~1.9-years-single-threaded / potentially-days-on-a-cluster ESTIMATE (INTERNAL, 2026-07-08 audit), the hardware-factor mitigation, and the Safari no-PRF gap. Docs-only; no UI surface (e.g. an in-app onboarding notice) added yet — owner may still refine wording or add an in-app surface pre-launch. | #753, #754 |
...
type guard) — see the table above.** **M-10 and L-5 are now BUILT (docs disclosures, 2026-07-12) — source comments added; see table rows above.** Remaining open items (M-1, M-4, M-8, M-9,
L-1, L-2, L-3) must not be marked resolved until the owner confirms a resolution.
M-1 is an architectural ethers v6 limitation with no available fix; M-8's
`assertSaneKdfParams` partial mitigation does not close the AAD gap and full binding
