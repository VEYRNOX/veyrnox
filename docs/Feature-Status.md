...
- Action Password — second knowledge factor at CRITICAL points (PIN + Password) — 🟡 BUILT / UNAUDITED-PROVISIONAL (PR #195; `src/lib/twoFactorGate.js` pure verdict, `src/wallet-core/actionPassword.js` persistable Argon2id record, stored **inside** the encrypted multi-vault container `src/wallet-core/multiVault.js` so it carries no on-disk tell, `src/lib/WalletProvider.jsx` hooks, `src/components/security/{TwoFactorGate,useActionGuard}.jsx`, Security Center "2FA" card). Enforced at: **send** (`SendCrypto.jsx`), **reveal recovery phrase** (`WalletPortfolioPage.jsx`), **set duress PIN** / **create hidden wallet** / **hide existing wallet** (`DuressPin.jsx`, `StealthWallets.jsx`). The two 192 MiB Argon2id checks (unlock-credential + Action Password) run **sequentially** (Defect-A: never two concurrent allocations); 5 wrong attempts → `lock()` (I4, identical in a decoy session so the lockout is not a tell). Opt-in: no Action Password set → unchanged behaviour. **HONEST SCOPE:** two knowledge factors on one device ≠ hardware 2FA; both are derived/stored on the same device. Enforcement is **active-set (primary) only** — see the decoy/hidden-parity TARGET in §6 and the design constraint in `docs/vault-auth-architecture-brief.md`. Not on-chain verified (no txid involved): BUILT, not "verified".
