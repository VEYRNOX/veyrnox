...
- Panic wipe (emergency local key destruction) — ✅ (`panic.js`; panic/wipe PIN at unlock + in-app guarded wipe; `inspectKeyMaterial()`). **Panic-wipe residue gaps CLOSED (PR #918, 2026-07-13) — BUILT / unit-tested, INTERNAL:** GAP-1 (HIGH) `veyrnox-passkey-signcount` added to `DENIABILITY_RESIDUE_KEYS` — passkey usage counter survived prior wipes (proved pairing usage); GAP-2 (HIGH) `veyrnox-decoy-biometric` added to `DENIABILITY_RESIDUE_KEYS` — decoy biometric credential key survived wipe, revealing decoy-vault existence; GAP-3 (MED) `RESIDUE_KEY_PREFIXES` + `readPrefixedResidueKeys()` added to sweep `veyrnox-snapshots-<fingerprint>` wildcard-prefix keys; GAP-4 (LOW) 9 app-usage tells added to `METADATA_RESIDUE_KEYS`. 23/23 tests. NOT device-verified, NOT independently audited, no on-chain txid.
...
- RASP native detection / remote attestation — 🟡 **Android native probe BUILT / device-unverified; iOS probe + remote attestation unbuilt.** The **Android** on-device probe source IS built and registered: `android/app/src/main/java/com/veyrnox/app/RaspIntegrityPlugin.kt` (258 lines, `registerPlugin(RaspIntegrityPlugin.class)` in `MainActivity`, JS-wired via `src/rasp/nativeProbe.js` → `RaspIntegrity.checkIntegrity()`) implements all four signals — `rooted` (su-binary + Magisk/KernelSU paths, system-writable, build-tags), `hookedProcess` (Frida default port 27042 connect-probe, Xposed), `emulator`, `tampered` — fail-honest (a denied/failed probe never fabricates `rooted=true`; `detect()` maps unavailable → `INTEGRITY_UNAVAILABLE` → WARN). **NOT device-verified — this is F-09:** the probe has never been run against a real rooted / Frida-hooked device (Phase 4; needs a rooted/Frida Android device + a human, not doable in JS/CI). The **iOS** Swift probe is genuinely **unbuilt** (no Swift RASP plugin file exists), and the **remote-attestation leg** (2b — Play Integrity / App Attest) is now **BUILT** (PROVISIONAL — `src/rasp/attestation.js` + `PlayIntegrityPlugin.kt` + `AppAttestPlugin.{h,m}`; RS256 on-device JWS signature verification **BUILT (PR #922, `verifyJwsSignature()`)** — x5c leaf cert extracted + `SHA256withRSA` verified on-device; **root-cert pinning absent** (no bundled Google root DER; treat result as PROVISIONAL until pinning lands)). Until the Android probe is device-verified and iOS/attestation land, native detection stays unverified and the dashboard reads `pending` (`RaspSecurity.jsx`). **Capability-gap context vs. commercial app shielding (Promon SHIELD):** `docs/rasp-vs-commercial-shielding.md` (2026-07-12) compares Veyrnox RASP against a commercial shield feature-by-feature and scopes the missing categories as a leverage-ranked gap roadmap — G1 production-build obfuscation/anti-tamper (**BUILT, PR #917**: G1a ProGuard `-repackageclasses`/`-allowaccessmodification` added to `android/app/proguard-rules.pro`; G1b `veyrnoxObfuscatorPlugin` added to `vite.config.js` using `javascript-obfuscator` string-array+base64 encoding under `VITE_RELEASE=1`; 12 structural-pin unit tests in `src/rasp/__tests__/g1-obfuscation-config.test.js`; honest limits: Rollup chunk filenames unrenameable, object-key mangling off for Capacitor bridge safety, commercial shielding (Promon/Guardsquare) remains TARGET), G2 remote attestation (egress leg **BUILT**: `src/rasp/attestation.js` + `PlayIntegrityPlugin.kt`; **RS256 on-device JWS signature verification BUILT (PR #922, `verifyJwsSignature()`)** — x5c leaf cert + `SHA256withRSA` check on-device; root-cert pinning absent (PROVISIONAL until bundled); see `docs/rasp-attestation-egress-decision.md`), G3 evasion-hardening + real hostile-device testing (**BUILT, PR #915**: 16 structural regression pins in `src/rasp/__tests__/g3-frida-chain.test.js` lock the full Frida→BLOCK detection chain in CI; device-gated Appium spec at `tests/android/specs/frida-hook-detection-e2e.spec.js`; **G3 Frida-detection DEVICE-VERIFIED (INTERNAL) 2026-07-13** — Samsung Galaxy Note 20 5G SM-N981B, Android 13/API 33, `com.veyrnox.app.debug`: frida-server 17.15.4 running on port 27042 (confirmed via `adb shell ss -tlnp`); `RaspIntegrity.checkIntegrity()` called via Chrome DevTools Protocol → `{"rooted":false,"hookedProcess":true,"emulator":false,"tampered":true}`; `hookedProcess:true` confirms the Frida port 27042 connect-probe fires on a real device; signal maps to `signals.hooked=true` → `CONDITION.HOOKED` → `TIER.BLOCK` per unit-tested chain; `tampered:true` is expected for this debug build — `RELEASE_CERT_SHA256` not set in Gradle (honest gap, same as F-09); `rooted:false` is Magisk Hide at probe level (expected); INTERNAL — not independently audited; broader G3 evasion hardening [non-default Frida port, renamed libs, ptrace] remains TARGET), G4 screen-capture + overlay defence (**BUILT, PR #888**: `FLAG_SECURE` set window-wide in `MainActivity.java:30-33` — blocks screenshots/screen-recording/recents thumbnail for the entire app incl. all seed-reveal surfaces; `setFilterTouchesWhenObscured(true)` on Capacitor WebView blocks overlay-phishing taps; 7 structural-pin unit tests in `src/rasp/__tests__/g4-android-flag-secure.test.js`; M13 device-verification spec at `tests/android/specs/flag-secure-screenshot-e2e.spec.js`; **M13 DEVICE-VERIFIED (INTERNAL) 2026-07-13** — Samsung Galaxy Note 20 5G SM-N981B, Android 13/API 33, build TP1A.220624.014.N981BXXSIHYH3, `com.veyrnox.app.debug`: `adb shell screencap` returned 27,857 bytes (M13-A, home screen) and 24,264 bytes (M13-B, post-WebView-settle) — both under the 50 KB all-black threshold, confirming FLAG_SECURE propagates into the Capacitor WebView surface and is not limited to native window chrome. A genuine app screenshot at this resolution is >200 KB. INTERNAL evidence only — not independently audited; iOS equivalent not yet wired; broader G4 [seed-reveal/export chokepoints] PLANNED), G5 independent audit. It is an ANALYSIS/design doc — nothing in it is verified; each gap still walks `docs/rasp-validation-roadmap.md` before it can be VALIDATED.
...
- **WC live-pairing supervised E2E spec (PR #919, 2026-07-13) — BUILT / unit-tested, INTERNAL, SUPERVISED:** `e2e/walletconnect-live-pairing.spec.js` added (4 tests, gated `RUN_SUPERVISED_E2E=1`); uses `@walletconnect/sign-client` as a scripted dApp peer to exercise H7 (EIP-712 chain binding), H8 (personal_sign address binding), and M11 (session expiry). Replaced 4 vacuous `expect(true).toBe(true)` Appium WC stubs with source-structure pins. WC live relay gap status: SUPERVISED (not CI-automated). NOT device-verified, NOT independently audited, no on-chain txid.
...
WalletConnect live-pairing still needs a real dApp/scripted peer (not yet tested); KDF perf
...
- **M1 (LOW) — open.** `hiddenBalance.js:151` throws a raw string, not `new
  ...
  fixed in PR #858.
- **L1 (LOW) — ✅ CLOSED (PR #921, 2026-07-13).** `computePortfolio`/`usePortfolio` (`portfolioBalances.js`) now has an explicit `isDeniabilitySessionActive()` guard as its first line — the provider-layer-only reliance gap is closed. ERC-20 `Contract.balanceOf()` I3 bypass also fixed in the same PR. See PR #921 entry below.
...
## PR #921 — I3 ERC-20 egress gaps + belt-and-suspenders (2026-07-13)

> ⚠️ BUILT · unit-tested · INTERNAL — NOT device-verified, NOT independently audited. No on-chain txid.

- **GAP-1 (HIGH) — FIXED (PR #921, 2026-07-13).** ERC-20 `Contract.balanceOf()` in `portfolioBalances.js` bypassed `getBalanceEth`'s `isDeniabilitySessionActive()` guard — live RPC calls fired in deniability sessions for any ERC-20 token balance read. Fixed fail-closed: now throws `I3_DENIABILITY_EGRESS` when a deniability session is active.
- **GAP-2 (FRAGILITY) — FIXED (PR #921, 2026-07-13).** `computePortfolio` now has an explicit `isDeniabilitySessionActive()` guard as its first line — closes the PR #858 L1 open item (which relied solely on provider-layer guards that H1 proved can diverge independently).
- **GAP-3 (DEFENSE-IN-DEPTH) — BUILT (PR #921, 2026-07-13).** `TransactionHistory`, `FeeAnalytics`, and `useAnalytics` `useQuery` `enabled` gates now include `!isDeniabilitySessionActive()`.
- **GAP-4 (BROKEN SPEC) — FIXED (PR #921, 2026-07-13).** `e2e/i3-deniability-egress.spec.js` re-enabled; host list expanded to include EVM RPC, BTC Esplora, SOL RPC, and price API endpoints.
- Tests: 21 new unit tests + e2e re-enabled. BUILT / unit-tested, INTERNAL — NOT device-verified, NOT independently audited. No on-chain txid.
