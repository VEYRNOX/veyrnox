...
- **Android:** StrongBox HMAC-SHA256 (StrongBox-**preferred**, NOT enforced — a TEE/software-backed AndroidKeyStore key is accepted and honestly surfaced; StrongBox enforcement remains TARGET, see the StrongBox-tier line below) + biometric re-enrollment invalidation (Fingerprint / StrongBox tied to unlock). ✅ **BUILT, end-to-end device-verified (2026-07-05, comprehensive E2E test suite: 49/49 tests PASSED on Google Pixel 10 Pro XL, Android 16/API 36, Appium automation — Vault 8/8, Send 2/2, Hardware KEK 5/5, Panic PIN 8/8, Hidden Wallet 8/8, Biometric Unlock 8/8, Send Scenarios 10/10 with multi-asset on-chain verification: Sepolia ETH, USDC, Bitcoin testnet, Solana devnet all confirmed on-chain; Hardware KEK badge "Hardware Protection ON" active on device).**
