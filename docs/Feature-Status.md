...
- **Security Advisor + Send Preview — Tornado Cash / Lazarus / Blender.io / Sinbad now BLOCK on every EVM chain (2026-08-09, wallet PR #1646 + tip PRs #37/#39) — ✅ BUILT, INTERNAL. Worker-level verification only: the curl matrix and the **Advisor** UI were observed against the deployed prod Worker; the **Send Preview** UI retest is OUTSTANDING (see the verification bullet below). Send Preview is the surface D2 broke, so its fix is evidenced by curl, not by the screen a user actually sees.** Three ordered defects discovered while regression-testing the router `0x8589427373D6D84E98730D7795D8f6f8731FDA16`, all shipped and re-verified end-to-end via curl against the deployed Supabase edge → prod TIP Worker.
  ...
  - **Honest gaps.** BUILT, INTERNAL, not device-verified in the strict on-chain-txid sense (screening advisories, not signing). Three sanction-grade Worker sources (Chainalysis, OpenSanctions, Alchemy tx-sim) remain unfunded on staging (`*_API_KEY not configured`). Not independently audited. #3 (Lazarus router in the regression sweep) tagged `Ronin Bridge Exploiter` on `etherscan-labels` — that namespace is NOT in `SANCTIONED_LABEL_NAMESPACES`; OFAC-GitHub + GoPlus caught it, but the curated list could be extended with a citable sanctioning-authority reference.
