...
## 2026-07-13 Android RASP improvements — Magisk-Hide-bypass detection vectors (BUILT-UNVALIDATED)

> ⚠️ BUILT-UNVALIDATED — code compiled, NOT yet re-tested on the SM-N981B or any Magisk device. INTERNAL, not independently audited. No on-chain txid. The 2026-07-12 DEVICE-VERIFIED status covers the original pre-sign gate flow and the three original checks only.
...
- **`checkDangerousProps()`** — reads `ro.boot.verifiedbootstate` and `ro.boot.flash.locked` via `getprop`. An unlocked bootloader reports `verifiedbootstate=orange` or `flash.locked=0`; these values come from the bootloader and are baked into the kernel command line, so Magisk Hide does not intercept them.
...
**Status: BUILT-UNVALIDATED.** A rebuild, install, and re-run of `checkIntegrity()` on the SM-N981B (or another Magisk device) is required before these new checks can be considered validated. Until re-tested, the F-09 row remains DEVICE-VERIFIED for the 2026-07-12 pre-sign gate flow with the original checks only.
