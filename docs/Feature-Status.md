...
| H-2 / iOS-F11 | HIGH | Android + iOS | Biometric factor not bound to enrollment set. **Scope (clarified per `docs/audit-triage/independent-audit-2026-07-06-android-kek-suite.md` F3.1):** this row is about **KEK-enrolled-vault** invalidation only — a property of the Keystore/Secure-Enclave HMAC key itself. It does **not** apply to, and makes no claim about, the plain app-layer "Biometric Unlock" toggle on a **bare** (non-KEK) vault, which is a JS-side `BiometricAuth.authenticate` prompt with no OS-enforced per-item ACL — see the "Biometric unlock" line above (§4) and the M2c/d decision note for that separate, narrower guarantee. **Android (KEK-enrolled vault): RESOLVED / device-verified** — `setInvalidatedByBiometricEnrollment(true)` ([HardwareKekPlugin.kt:199](../android/app/src/main/java/com/veyrnox/app/HardwareKekPlugin.kt)) confirmed working on Pixel 10 Pro XL 2026-07-01 (PR #516/#518): re-enroll → `KeyPermanentlyInvalidatedException` → fail-closed → PIN recovery. **iOS (KEK-enrolled vault): DEFERRED (device-blocked, MDM)** — `kSecAccessControlBiometryCurrentSet` is correctly set on the SE key ACL ([HardwareKekPlugin.m:96](../ios/App/App/HardwareKekPlugin.m)) but the runtime re-enroll test could not be run: the test iPhone 17 Pro Max is MDM-registered, and the MDM profile restricts Face ID enrollment changes. Attempted 2026-07-07 during the iOS KEK device session — confirmed MDM-blocked. Needs a different, unrestricted iPhone. Note: the separate `biometricUnlock.js` PIN cache on a bare vault remains H-NEW-5 (plugin cannot set the ACL) — honest-disabled. | Android ✅ device-verified for KEK-enrolled vaults (PR #516/#518). iOS OPEN — re-enroll test on an unrestricted iPhone (MDM-blocked on current test device, confirmed 2026-07-07). Bare-vault app-layer biometric gate is a separate, narrower guarantee — see H-NEW-5. |
