...
> binding. **Status: C-1 REGRESSED / binding-unconfirmed (2026-07-05 finding).** A v3 fix
> is in progress on branch `claude/silly-knuth-0e85fc`; device re-verification, a new txid,
> and a salt-tamper negative test are required before "RESOLVED" is claimed again. See the
> dated annotation in `docs/audit-2026-07-01-kek-internal.md`.
...
|---|---|---|---|
| H-NEW-D | iOS + Android native / KEK | **CLOSED (native-layer SE ECIES confirmed)** — 2026-07-01 INTERNAL static-analysis pass confirmed `kSecAttrTokenIDSecureEnclave` is present in `HardwareKekPlugin.m:78`. The SE ECIES design is correctly implemented at the native ObjC layer. iOS device-verified status remains BUILT/device-verified (PARTIAL): two Sepolia sends confirmed on-chain from KEK-enrolled vault (PR #495), but the live SE-unlock log trace tied to those sends was not captured (iOS-F9, below). Android is BUILT/end-to-end device-verified on the StrongBox-gated unlock FLOW (Pixel 10 Pro XL, PRs #497 #499) with a KEK-gated Sepolia send confirmed on-chain (txid `0x9d9ff549…`, block 11180398) and its biometric re-enrollment invalidation test PASSED (PR #516/#518). Outstanding: biometric re-enrollment invalidation test on iOS only (device-blocked); iOS SE-unlock log trace tied to a send (iOS-F9 — iOS has txids but no observed SE-unlock line); independent audit. Android additionally: C-1 CRITICAL (HMAC fixed input) — recorded RESOLVED / device-verified 2026-07-02 (PR #529), **REGRESSED 2026-07-05**: the per-enrollment `kekSalt` binding is cryptographically unconfirmed on-device (facade arg-drop + bridge JSON.stringify silently reverted enroll+unlock to the fixed v1 salt); v3 fix in progress, see the C-1 row below and `docs/audit-2026-07-01-kek-internal.md`. H-1 (StrongBox tier surfacing) FIXED PR #527. | H-NEW-D native-layer gap CLOSED; remaining gates: iOS SE-unlock log trace (iOS-F9) + biometric re-enrollment test (iOS only — Android PASSED PR #516/#518) + independent audit + Android C-1 v3 salt-binding fix/device re-verification (REGRESSED 2026-07-05). Android KEK-gated unlock-FLOW on-chain txid DONE (`0x9d9ff549…`) |
