...
- [x] **⚠ OWNER-DECISION 6** — **RESOLVED (2026-07-20) by owner: NOT designed for children — target audience 18+ only.** Play Console → App content → **Target audience and content**: select adult age bands only; answer **No** to "designed for children"; do **not** opt into the Families Policy programme (Veyrnox could not meet its ads/data/content rules, and a self-custody wallet is not child-directed). **Backed by an existing eligibility clause on BOTH legal surfaces** (verified 2026-07-20, identical wording): in-app `src/pages/TermsLegal.jsx:191` §4 "Eligibility", and the public site `https://veyrnox.com/terms` §4 (verified by rendering the page — the site is a client-rendered SPA, so `curl` returns only the shell and cannot confirm this; a rendered read is required):
  > *"Be at least 18 years of age (or the age of legal majority in your jurisdiction, whichever is higher) and have the legal capacity to enter into binding agreements."*
  The same clause also carries sanctions/OFAC-SDN screening, lawful-use, and legally-obtained-funds requirements — so the 18+ Play declaration is substantiated by published terms, not just an assertion on the form.
...
- [x] **⚠ OWNER-DECISION 8** — **RESOLVED (2026-07-20): `https://veyrnox.com/data-deletion`** — supplied by owner and **verified live**: `curl` returns **HTTP 200**, zero redirects, and serves real per-route content (page title "Data Deletion", not the SPA fallback shell). Enter this as the **data deletion URL** in Play Console → App content → Data safety. **Play form field (§Overview gate 3 wording):** *"Users can irreversibly destroy all local key material and vault data on-device via Panic Wipe (`src/wallet-core/panic.js`). No server-side user record exists to delete — Veyrnox has no accounts; the seed is the identity. On-chain transaction history remains public on the blockchain and cannot be deleted by anyone, an honest limit stated on the deletion page and the TermsLegal screen."*
  > **Re-verify before each submission** that the URL still returns 200 — a dead deletion URL is a policy strike.
...
- [x] **Follow-up (docs honesty)** — DONE (PR #1251, 2026-07-20): CLAUDE.md's RC-audit "explicitly NOT added" list corrected; `setAttributes` removed from it and documented with honest scope (see ⚠5).
...
- **Updated:** 2026-07-20 — resolved ⚠1, ⚠3, ⚠5, ⚠7, ⚠9 from code/hard-rules
  (**9 of 9** numbered owner-decisions now resolved, up from 2 — only counsel review and a
  submission-day re-verify remain; ⚠6 18+/not-for-children confirmed by owner and backed by
  the §4 Eligibility clause on both legal surfaces; ⚠8 data-deletion URL verified live (200); ⚠9 verified against the signed
