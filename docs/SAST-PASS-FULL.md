...
- **Crypto core re-confirmed (prior hardening intact, not undone by refactors):**
  - Argon2id `KDF_PARAMS` = 192 MiB / 3 iters / parallelism 1, `Object.freeze`d (`vault.js:46-52`); *(note: KDF reverted to 64 MiB post-audit; see commit 1226085e; line reference corrected — vault.js:51-56 is now KEK_C derivation, not KDF_PARAMS)* legacy 64 MiB floor used only to decrypt old blobs via their own recorded params — no downgrade path. Decoy/chaff KDFs spread the **same** `KDF_PARAMS` (`deniabilityUnlock.js:94-102`, `stealth.js:255`) — no timing tell.
