...
- **Chaff unopenability rests on CONSTRUCTION, not on a test.** The guarantee that no enterable PIN opens a chaff slot is that the password comes from `crypto.getRandomValues` and is immediately discarded — nothing maps an enterable PIN to it. The three-PIN unopenability test (`''`/`000000`/`123456`) is a regression guard on that construction, not a proof over the 10^6 PIN space. The reliance is on the random-and-discarded construction.
