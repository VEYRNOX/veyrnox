...
- ☐ ENS resolution — not yet built (Phase D / future scope). Documented; not a current send-path defect.
...
3. Acknowledge: ENS resolution not built — known scope gap (Phase D), non-blocking for current send path.
