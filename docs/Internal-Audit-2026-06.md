...
| H-2 PIN sole at-rest factor (web) | HIGH | 🟡 PARTIAL MITIGATION (still OPEN) | Hardware-bound KEK (native + audit) is the named long-term fix and remains unbuilt. **Partial defence-in-depth mitigations landed 2026-06-27 (PR #418 + #424):** (1) H-A: `validateWebVaultPassword()` enforces a ≥12-character minimum for web vault creation on mainnet (`ALLOW_MAINNET = true`) — raises keyspace above the 8-digit PIN 10^8 baseline, but a user-chosen password is still offline-exhaustible. (2) H-NEW-4/H-NEW-6: KEK zeroing in `try/finally` — reduces heap-residue window after the unlock path. **Neither closes H-2.** Status stays HIGH/OPEN; gate conditions unchanged. See `docs/audit-triage/kek-acl-rasp-status-gate-2026-06-22.md` §5a. |
