...
(function () {
  ...
  // Painted == at least one React descendant of #root occupies real pixels.
  // Deliberately ignores #root itself (see note 1 above), plus the static
  // boot shell: it prevents an initial white screen but is not React UI.
  function hasVisibleContent(root) {
    if (!root) return false;
    var els = root.querySelectorAll('*');
    var limit = els.length < MAX_NODES ? els.length : MAX_NODES;
    for (var i = 0; i < limit; i++) {
      if (els[i].id === 'boot-shell' || els[i].closest('#boot-shell')) continue;
