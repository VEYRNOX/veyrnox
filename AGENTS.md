# Veyrnox — project guide for Codex
...
- **Subagents** (`.Codex/agents/`): `veyrnox-recon` (read-only mapping + root cause),
  ...
  same file.
- **Command** (`.Codex/commands/parallel-fix.md`): `/parallel-fix <area>` — recon → fan out
  one implementer per independent item → honest review → integrate & verify.
- **Workflow** (`.Codex/workflows/branch-review.js`): run the `branch-review` workflow to
