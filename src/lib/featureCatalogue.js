  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        status: 'built',
        summary: 'iOS Secure Enclave ECIES + Android StrongBox HMAC KEK — device-verified (PARTIAL), UNAUDITED (internal pass only, NOT independent)',
        explanation: 'Built. Native hardware Key-Encryption-Key that wraps the PIN-derived vault DEK under KEK = HKDF-SHA256(H ‖ C) + AES-256-GCM, so an offline-seized vault blob cannot be PIN-exhausted without the device: the hardware factor H is released only per-use behind biometric auth and never leaves the secure element. iOS: Secure Enclave P-256 ECIES (non-extractable key, .biometryCurrentSet biometric ACL) shipped as a native Objective-C plugin (PR #495, registration blocker resolved via the two-file CAPPlugin split) and device-verified (PARTIAL) on iPhone 17 Pro Max. Android: AndroidKeyStore HMAC-SHA256, StrongBox-preferred but NOT enforced (honest tier reporting), device-verified on Pixel 10 Pro XL. NOT "verified": the biometric re-enrollment invalidation test (both platforms) and the live iOS SE-unlock trace remain outstanding. OPEN CRITICAL (C-1): Android SOFTWARE-tier keys are not yet refused — a software-tier key can satisfy KEK enrollment with no hardware element, breaking the I6 invariant; this is not a "known gap", it is an open CRITICAL finding (docs/audit-2026-07-01-kek-internal.md). AUDIT STATUS: internal static-analysis pass only (2026-07-01); NOT an independent audit; "internal" must never be presented as "independent" (I4). Independent audit required before this control can be promoted. See docs/Feature-Status.md §4.',
      },
      {
