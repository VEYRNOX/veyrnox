  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        status: 'built',
        summary: 'Optional extra OS-biometric check at every unlock, in addition to your PIN and device-protected key. Face on iOS (face-strict); any strong biometric on Android (no public API to require face specifically).',
        explanation: 'Opt-in in Settings → Security. When enabled, WalletProvider.unlock() runs an extra gate AFTER the PIN and hardware-KEK factors have both succeeded and BEFORE the visible session mounts: (1) RASP fresh probe — a WARN or BLOCK tier fails the unlock closed without prompting; (2) OS biometric via verifyBiometric2fa (Face ID on iOS, BiometricPrompt Class 3 on Android) — a cancel, mismatch, lockout, or unavailable is treated as declined and fails closed (I4); (3) iOS post-check — the cached biometryType MUST be faceId or the gate rejects with the machine code `requires-face` (Touch-ID-only iPhones cannot enrol). No face template is stored by VEYRNOX — this is purely a gate signal from the OS. Gated on isPrimary (K-2): the gate is a silent no-op on decoy and hidden sessions, so a coerced unlock never observes that Theft Protection is configured. The setting write is I3-guarded — a deniable/demo session cannot mutate the real user\'s preference (matches setBiometricUnlockEnabled). Android accepts any BIOMETRIC_STRONG biometric because there is no public API to force face-only — copy documents that asymmetry honestly. Reuses the existing @aparajita/capacitor-biometric-auth plugin path; no new native plugin was written. STATUS: BUILT — code-shipped and unit-tested (theftProtection.gate.test.js, 15 assertions; WalletProvider.theftProtection.test.js pins the call-site wiring). NOT device-verified on iOS or Android, and NOT independently audited.',
      },
      {
