  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        status: 'built',
        summary: 'Manual ERC-20 tracker — curated quick-add + custom entry',
        explanation: 'Built as a manual token manager (/erc20-discovery): 12-token curated quick-add list (USDC, USDT, DAI, WBTC, WETH, LINK, UNI, AAVE, MKR, COMP, CRV, MATIC) plus a custom-entry form with optional contract address. Tokens stored in local IndexedDB (WalletToken entity). No auto-scan — no address is egressed to a third-party indexer. Spam detection via annotateTokens(). USD values I2-gated.',
      },
      {
        name: 'Additional Networks',
        status: 'built',
        summary: 'Network Manager: custom RPC endpoint CRUD',
        explanation: 'Built (/network-manager): add, edit, enable/disable custom EVM RPC endpoints stored in local IndexedDB (NetworkConfig entity). No live RPC calls from the manager UI itself. Adding a new chain here wires it into the wallet\'s provider stack. Non-EVM stacks (each a separate cryptographic stack + audit) remain on the roadmap.',
      },
    ],
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        explanation: 'Auto-lock on idle timeout and app backgrounding is implemented in the wallet unlock lifecycle. A session view surfaces active sessions. Built and working in the testnet/provisional build.',
      },
      {
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        status: 'built',
        summary: 'Aggregate crypto holdings net-worth view',
        explanation: 'Built (/net-worth): aggregates holdings across wallets and chains for a net-worth overview. On-device computation over locally-cached balances; USD values I2-gated behind the live prices opt-in.',
      },
      {
        name: 'P&L Tracking',
        status: 'built',
        summary: 'Realised / unrealised profit and loss',
        explanation: 'Built (/pl): per-asset P&L using real histoday price data from CryptoCompare. USD display is I2-gated — no price data is fetched until the user opts in to live prices. Shows "—" when off or unavailable. Read-only; no fiat, no financial advice.',
      },
      {
        name: 'On-Chain Analytics',
        status: 'built',
        summary: 'Transaction analytics from on-device history',
        explanation: 'Built (/onchain — retitled Transaction Analytics): insights from the wallet\'s own transaction history stored on-device. Honest scope note: no third-party indexer; EVM history uses the same provider chain as the balance and history views. Read-only, no new egress.',
      },
      {
        name: 'Fee Analytics',
        status: 'built',
    ...
    features: [
      {
        ...
        explanation: 'Built (/price-charts): real OHLCV candlestick data from CryptoCompare histoday API. I2-gated — no price fetch until the user opts in to live prices. Shows an opt-in prompt when off. Read-only; no trading.',
      },
      {
        name: ‘Price Alerts’,
        status: ‘built’,
      ...
      },
      {
        ...
        explanation: 'Built (/push): in-app notification centre with push-delivery configuration. Wallet and security events surface here.',
      },
    ],
  },
  {
    ...
    features: [
      {
        ...
        explanation: 'Built (/nft): display-only NFT gallery backed by local IndexedDB. USD sub-label I2-gated via useLivePrices(). No minting, fractionalization, or marketplace. ETH-denominated P&L is on-device.',
      },
      {
        name: 'Multi-Chain NFT Viewing',
        status: 'built',
        summary: 'View NFTs across chains (manual tracking)',
        explanation: 'Built (/nft-multichain): NFT portfolio view across chains backed by local IndexedDB. All values in ETH only (no USD conversion, no stale rate). Manual tracking; honestly labelled.',
      },
    ],
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        explanation: 'Built (/crypto-signing): sign plain messages or derive keys using local ethers.js (v6) — all client-side, no external call. Handles signMessage(), signTransaction(), and HD derivation for standard proof-of-ownership use cases. Distinct from arbitrary dApp signing.',
      },
      {
        name: 'Split Bill',
        status: 'roadmap',
      ...
      },
      {
        ...
        explanation: 'Built (/payment-links): PaymentLink records stored in local IndexedDB. The shareable URL is constructed from the user-entered address + amount — no external call, no payment processing, no server.',
      },
      {
        name: 'Recurring Payments',
        status: 'built',
        summary: 'Self-initiated schedule reminders',
        explanation: 'Built (/recurring): CRUD for recurring payment schedules stored on-device. Honest non-custodial banner; no auto-debit path — each occurrence hands off to /send for the user to sign manually. The wallet never moves value autonomously.',
      },
    ],
  },
  {
  ...
  },
  {
    ...
    features: [
      ...
      },
      {
        ...
        explanation: 'Built (/voice-commands): uses browser-native SpeechRecognition / webkitSpeechRecognition. Command matching and routing are local (phrase map + React Router). No audio leaves the device; the page discloses this explicitly. Degrades gracefully when the browser API is absent. Never used for unattended signing.',
      },
    ],
  },
];
