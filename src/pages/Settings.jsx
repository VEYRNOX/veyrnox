  ...
  return (
    <div className="max-w-lg mx-auto space-y-8">
      ...
      {/* AI Security Advisor — online answers. The privacy policy (§9) tells the
          ...
          shipped before this control existed, which is the same defect the
          telemetry comment above warns about — a promise no surface delivers.
          lib/advisorConsent.js already had the writers; nothing called them.

          Asymmetric on purpose. Switching OFF records an explicit denial and
          stops egress immediately (sendMessage re-reads consent per call, so
          there is nothing to flush). Switching ON does NOT grant — it CLEARS the
          stored answer, so the Advisor shows its full disclosure again and the
          grant is made there, with the explanation in front of the user.
          Granting silently from a toggle would be consent without disclosure,
          which is what §9 exists to prevent.
