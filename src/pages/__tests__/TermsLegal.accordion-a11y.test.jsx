...
describe('TermsLegal — accordion trigger<->panel association', () => {
  ...
  // 29 = 15 terms sections + 14 privacy-policy sections. The privacy policy was
  ...
  it('each of the 29 sections gets its own distinct panel id (no collisions)', () => {
    renderPage();
    const triggers = screen.getAllByRole('button').filter((b) => /^\d{1,2}\./.test(b.textContent));
    expect(triggers.length).toBe(29);
    const ids = new Set();
