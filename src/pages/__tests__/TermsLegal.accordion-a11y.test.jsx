...
describe('TermsLegal — accordion trigger<->panel association', () => {
  ...
  // 28 = 15 terms sections + 13 privacy-policy sections. The privacy policy was
  ...
  it('each of the 28 sections gets its own distinct panel id (no collisions)', () => {
    renderPage();
    const triggers = screen.getAllByRole('button').filter((b) => /^\d{1,2}\./.test(b.textContent));
    expect(triggers.length).toBe(28);
    const ids = new Set();
