...
const features = [
  ...
  { category: "Networks & Assets", icon: Coins, items: [
    ...
    { name: "Additional Tokens", desc: "Manual ERC-20 tracker: 12-token curated quick-add + custom entry via /erc20-discovery — no auto-scan, no address egress", status: "available" },
    { name: "Additional Networks", desc: "Network Manager: add, edit, enable/disable custom EVM RPC endpoints (local IndexedDB; no new chain stacks)", status: "available" },
  ]},
  { category: "Access & Authentication", icon: KeyRound, items: [
    ...
    { name: "Hardware Wallet", desc: "Ledger WebHID address read + watch-only wallet import; Trezor Safe 5 setup guide with iOS/Android/desktop compatibility. Cold-key transaction signing is post-audit.", status: "available" },
  ]},
  ...
  { category: "Portfolio & Analytics", icon: BarChart3, items: [
    { name: "Portfolio Dashboard", desc: "Read-only net-worth view across wallets and chains; opt-in live prices, fails honest when off", status: "available" },
    { name: "Net-Worth Tracker", desc: "Aggregate holdings net-worth view across chains", status: "available" },
    { name: "P&L Tracking", desc: "Realised / unrealised P&L; opt-in live prices via CryptoCompare, I2-gated (no prices egressed until user opts in)", status: "available" },
    { name: "On-Chain Analytics", desc: "Transaction analytics from on-device history (honest scope: no third-party indexer)", status: "available" },
    { name: "Fee Analytics", desc: "Native-unit fee totals from chain history; no fiat, no persistence, no new egress", status: "available" },
  ...
  { category: "Prices & Alerts", icon: Bell, items: [
    { name: "Price Charts", desc: "Historical price charts", status: "available" },
    { name: "Price Alerts", desc: "Threshold notifications (advisory; never trades)", status: "available" },
    { name: "Watchlist", desc: "Track assets you don't hold", status: "available" },
    { name: "Notifications & Push", desc: "Notification centre + push delivery", status: "available" },
  ]},
  { category: "NFTs", icon: ImageIcon, items: [
    { name: "NFT Gallery (Display-Only)", desc: "View owned NFTs; no minting or marketplace", status: "available" },
    { name: "Multi-Chain NFT Viewing", desc: "View NFTs across chains (display only, manual tracking)", status: "available" },
  ]},
  { category: "Payments & Utilities", icon: CreditCard, items: [
    ...
    { name: "Message Signing", desc: "Sign arbitrary messages for proof-of-ownership (local ethers.js — no external call)", status: "available" },
    { name: "Split Bill", desc: "Split a cost; each pays from their own wallet", status: "roadmap" },
    { name: "Payment Links", desc: "Shareable request-to-pay link / QR (no processing; stored locally)", status: "available" },
    { name: "Recurring Payments", desc: "Self-initiated schedule/reminder; hands off to /send for user signing each time (no auto-debit)", status: "available" },
  ]},
  ...
  { category: "Platform", icon: Smartphone, items: [
    ...
    { name: "Voice Commands", desc: "Hands-free read-only actions via browser SpeechRecognition — no audio leaves device; never unattended signing", status: "available" },
  ]},
];
