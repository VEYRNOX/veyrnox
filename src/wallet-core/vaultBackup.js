...
// NOTE: A prior `restoreWithPassword(envelope, password)` export was removed
// (issue #1101). It decrypted the backup password seal and then called the web
// `saveVault()` directly, bypassing `getKeyStore().createVault()` — on native
// that silently wrote to web storage instead of SecureStorage / the KEK-wrapped
// vault. It had no production caller (the real path is
// `decryptPasswordSeal` / `decryptPinSeal` → `finalisePinRestore`, which routes
// through `getKeyStore().createVault()` and keeps the on-device vault in the
// PIN cohort). Do NOT reintroduce a password-seal restore that persists via
// `saveVault()`; route through `finalisePinRestore` instead.
