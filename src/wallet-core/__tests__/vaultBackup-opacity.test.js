...
describe('backup file is a BINARY encrypted-vault container (guards PR #239/#245)', () => {
  ...
  it('REAL crypto: binary file round-trips and both seals still decrypt', async () => {
    ...
    const env = await createBackupEnvelope(container, 'backup-pass-123', '246810');
    ...
    expect(await decryptVault(parsed.seals.pin, '246810')).toBe(container);
  }, 60000);
  ...
  it('verifyBackupEnvelope passes for the chosen creds and throws otherwise', async () => {
    ...
    const env = await createBackupEnvelope(container, 'GoodBackupPw1', '987654');
    await expect(verifyBackupEnvelope(env, 'GoodBackupPw1', '987654')).resolves.toBe(true);
    await expect(verifyBackupEnvelope(env, 'WrongPw', '987654')).rejects.toThrow(/verification failed/i);
    await expect(verifyBackupEnvelope(env, 'GoodBackupPw1', '000000')).rejects.toThrow(/verification failed/i);
  }, 60000);
});
