...
describe('backup file is a BINARY encrypted-vault container (guards PR #239/#245)', () => {
  ...
  it('REAL crypto: binary file round-trips and both seals still decrypt', async () => {
    ...
    const env = await createBackupEnvelope(container, 'backup-pass-123', '24681012');
    ...
    expect(await decryptVault(parsed.seals.pin, '24681012')).toBe(container);
  }, 60000);
  ...
  it('verifyBackupEnvelope passes for the chosen creds and throws otherwise', async () => {
    ...
    const env = await createBackupEnvelope(container, 'GoodBackupPw1', '98765432');
    await expect(verifyBackupEnvelope(env, 'GoodBackupPw1', '98765432')).resolves.toBe(true);
    await expect(verifyBackupEnvelope(env, 'WrongPw', '98765432')).rejects.toThrow(/verification failed/i);
