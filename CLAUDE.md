...
- **Testnet only.** Mainnet is gated behind the independent audit. `ALLOW_MAINNET = false`.
  Never enable a mainnet path or test against mainnet funds.
...
- **Status tags.** Every control/feature is BUILT (in code, testnet/provisional), TARGET
  ...
  most, never "verified".
- **Audit gate (§24).** The independent audit reviews the architecture BEFORE any backend
  or seed-touching build. RASP, hardware KEK, device attestation, network hardening,
  cloud recovery, and inheritance are TARGET/PLANNED — do not build them blind; they need
  real-device verification and the audit.
